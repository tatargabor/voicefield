...
- **No audio leaves the phone** — STT runs client-side (pluggable provider: Soniox, Deepgram, etc.)
...
# Plus your STT provider SDK, e.g.:
...
const { GET, POST, OPTIONS } = createVoicefieldHandler({
  generateSTTKey: async () => {
    // Return a temp key for whatever STT provider you use
...
**Why not just use the browser's SpeechRecognition API?** Browser support is inconsistent, accuracy varies wildly, and it doesn't work cross-origin. Voicefield uses the phone's microphone (better hardware) with a pluggable STT provider (Soniox by default, or bring your own), giving consistent, high-quality results across all devices.
